 ...
 *        is filled with random integers from 0-99 and also printed out
 *        to the screen.
 */
...
void loop() {
  ...
  if(sw1Event == SW_CLICK_LONG){      // Checks to see if switch 1 has been long clicked
    ...
    Serial.print("[LongClick] ");     // random integers from 0-99 and then printed out
...
// Helper functions
void fillArray(int arr[], int arrayLength){   // Fills the array with random numbers from 0-99
                                              // and then calls the printArray function.
