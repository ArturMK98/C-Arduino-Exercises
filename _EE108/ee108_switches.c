...
static SwitchInfo sw1Info = { PIN_SW1, 1, 1, 0, 0, 0 };
SwitchPtr sw1Object = &sw1Info; // point at information for switch1

static SwitchInfo sw2Info = { PIN_SW2, 1, 1, 0, 0, 0 };
SwitchPtr sw2Object = &sw2Info; // point at information for switch1
 ...
 * state
 *
 * @param debounceMs_  the new debounce time to use in ms
 */
 ...
 *
 * @param longClickMs_   the long click duration threshold
 */
void setSwitchClickDurations(unsigned int longClickMs_) {
...
//    timeNowMs -= 1;
  ...
  if (sw->debouncedSwitchValue == SW_ACTIVE) { // is debounced switch pressed?
    ...
    if (debouncedValueChanged) { // just released?
      ...
      // serialCPrintf("OFF: prevStateStateTimeMs changed to %u\n", sw->debouncedChangeTimeMs);
      sw->prevStateStartTimeMs = sw->debouncedChangeTimeMs;
    }
  }
